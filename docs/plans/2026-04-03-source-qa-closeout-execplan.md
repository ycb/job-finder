...
- [~] (2026-04-03 03:35Z) Fix queue/accounting mismatch so `Imported`, `New`, and `Unread` reflect the same latest-batch semantics and do not imply missing jobs. Partial: latest batch stamping now matches imported-kept rows exactly (`45` rows in batch `b6a544e9-4aa8-4a38-8434-7f00471c80d4`, matching latest-run imported totals), but the QA dashboard still shows fewer `New` rows because `New` is counting active-queue-eligible rows only (`20` after excluding low-signal and terminal rows). Remaining: decide whether to align UI semantics or make the difference explicit in the source/queue model.
