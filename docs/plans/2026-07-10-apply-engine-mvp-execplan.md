...
- [x] (2026-07-11 00:50Z) Milestone UX-0 + M5 surface (`11eb110`, `2c2774b`, `c416e83`): reconciled `queueBreakdown` (mutually exclusive buckets summing to stored; user actions outrank scoring gates) exposed via `queueMeta`; explanatory empty states ("You're through your queue" with the full account + view shortcuts; filter-hidden case with Clear filters); sources table shows one run context per row (last-run funnel, "N in library" secondary, "of ~N listed" expected context with <60% capture warning); scores in bucket language ("High signal · 85", "n / 100"); unified copy. QA-infra fix: follow loop restarts the review server on new commits, not only branch changes. Live-verified in the stakeholder's browser at 4311 with screenshots: worked-through-queue state renders real numbers (357 stored: 167 applied, 30 skipped, 2 rejected, 158 hard-filtered), View-applied shortcut navigates, Match labels render. Suite 534 pass / 0 fail. Remaining M5 tail: populate expected counts for sources whose capture paths don't yet record portal totals (UI already renders honest absence).
