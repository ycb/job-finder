...
- [x] (2026-03-31 23:44Z) Fixed a latest-run aggregation bug where later `cache_fresh` replays of the same `captured_at` could outrank the live row that actually produced the capture. Added explicit `latestTrustedRun*` metrics to source rows so the UI can render cumulative totals with inline latest-run `+X` deltas instead of forcing the user to infer recency from cumulative totals alone.
...
- [x] (2026-04-01 03:28Z) Reopened the Zip regression against the newest live QA evidence and traced it to a second self-inflicted builder drift: commit `2991c42` had moved `distanceMiles`, `datePosted`, and `minSalary` back into Zip URL constraints. Reverted those Zip criteria to honest post-capture evaluation, corrected page-two URL generation to keep `?page=2` instead of `/jobs-search/2`, and wired the generic board extractor to respect source-specific timeout overrides.
- [x] (2026-04-01 04:18Z) Forced a fresh LinkedIn live capture after restoring bounded detail reads. Capture volume improved from `28/56` to `35/56`, proving the source is still querying the right surface, but the artifact remained snippet-heavy: only `4/35` rows contained anything beyond `Company · Location`, and `0/35` descriptions contained `ai`.
- [x] (2026-04-01 04:32Z) Traced the remaining LinkedIn quality collapse to the detail-read interaction itself. The safer re-enable in `eb4f517` was clicking canonical job links, which preserved provenance but failed to populate the search detail pane. Switched the bounded detail-read path back to card clicks while only accepting detail text when the detail-pane job id matches the card job id.
- [x] (2026-04-01 05:04Z) Traced the remaining LinkedIn contamination to a second bug in the same extractor: even when the detail-pane job id did not match the card id, `readDetailHints()` still returned the stale pane description in its fallback path. Tightened the mismatch rule so unmatched or id-less detail panes return blank detail hints, and stopped pagination once the next page offset would exceed the live `expectedCount`.
- [x] (2026-04-01 05:19Z) Proved the `expectedCount` stop condition alone was not sufficient in live QA: LinkedIn still navigated to `start=50` when the count parser lagged. Added a second paginator guard that stops after any short page (`<25` cards), which matches the native page-size boundary and avoids the empty terminal page even when `expectedCount` is unavailable mid-run.
- [x] (2026-04-02 10:20Z) Re-validated the current ZipRecruiter path against live QA after the LinkedIn work settled. The active builder now emits the broad manual-equivalent URL `https://www.ziprecruiter.com/jobs-search?search=Product+manager+ai&location=San+Francisco%2C+CA&page=1`, and a fresh `capture-source-live zip-ai-pm --force-refresh` run captured `60` plausible jobs. This removes Zip from the primary regression slot.
- [x] (2026-04-02 10:26Z) Re-validated the current Indeed path against live QA. The builder emits `https://www.indeed.com/jobs?q=Product+manager+ai&l=San+Francisco%2C+CA&radius=25&salaryType=%24200%2C000&fromage=3`, and a fresh `capture-source-live indeed-ai-pm --force-refresh` run captured `19` plausible jobs without salary/career page pollution.
- [ ] Deferred follow-up after LinkedIn stabilization: audit JobFinder-to-source query-construction parity across the MVP slate, with explicit manual/native checks for ZipRecruiter date-posted and radius behavior plus a per-source gap list for any criteria that still drift from the JobFinder search form.
...
- Observation: the current Zip regression evidence is now stale.
  Evidence: a fresh 2026-04-02 live capture from `/Users/admin/job-finder` produced `pageUrl=https://www.ziprecruiter.com/jobs-search?search=Product+manager+ai&location=San+Francisco%2C+CA&page=1` and `jobs=60` with plausible top roles (`Principal Product Manager, GPUs and AI Accelerators`, `Product Manager - Conversational AI`, `Senior Product Manager, AI`, etc.). The remaining Zip work is query-surface analysis, not extraction rescue.
...
- Observation: the Indeed catastrophic regression is also stale.
  Evidence: a fresh 2026-04-02 live capture from `/Users/admin/job-finder` produced `pageUrl=https://www.indeed.com/jobs?q=Product+manager+ai&l=San+Francisco%2C+CA&radius=25&salaryType=%24200%2C000&fromage=3` and `jobs=19`, with plausible top roles from OpenAI, Uber, Google, and other Bay Area employers. Salary/career pages are no longer entering the capture set.
...
- Decision: ZipRecruiter and LinkedIn are the first fix priorities.
  ...
  Date/Author: 2026-03-31 / Codex
- Decision: after the 2026-04-02 live validations, LinkedIn remains the recently fixed critical source, but the next highest-value work shifts from Zip rescue to closing the broader source-map/manual-parity audit and the remaining run-honesty gaps.
  Rationale: fresh live captures show Zip (`60`) and Indeed (`19`) are currently returning plausible result sets, while LinkedIn extraction has already been restored to `82 / 82`. The primary remaining risk is not a single dead source; it is proving and documenting parity across all active sources and fixing smaller honesty/reporting gaps.
