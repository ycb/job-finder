...
- If `review:qa` is the accepted stakeholder QA entrypoint, its stop path must also shut down the LaunchAgent-backed `review:follow` parent. Killing only child `review`/watch processes is insufficient because launchd will immediately reclaim `4311`.
- `review:qa` should stop the 4311 review/watch/follow stack, but it should preserve an already-healthy bridge on `4315`. Restarting the bridge unnecessarily can turn a working QA session into a false infrastructure failure.

## QA Source Disable Consistency

- When removing a source from the stakeholder QA/MVP set, update both `/Users/admin/job-finder/config/sources.json` and `/Users/admin/job-finder/data/user-settings.json` (`onboarding.selectedSourceIds`).
- If only the config map changes, the dashboard can still surface the removed source through persisted onboarding state and make QA look inconsistent.
- When Chrome AppleScript reports JavaScript automation disabled, first verify the exact Chrome profile/window being targeted before concluding the global browser setting is wrong. Chrome profile drift can make a correct Developer setting look disabled from the automation process.
- When a user reports "no sites loaded," verify the dashboard payload and the default UI tab before touching backend ingestion. In this case `/api/dashboard` had sources and queue data, but the default `Search` tab hid the sources table and looked broken.
- When live browser sources suddenly fail with AppleScript errors after branch merges, verify and restart the long-running bridge/review processes before code changes. A stale `bridge-server` process can keep executing old provider code and produce false regressions (for example `-2740` syntax errors and zero imports).
- For ZipRecruiter, do not strip query params from captured job URLs down to only `lk`/`uuid`. Those extra params can be required for link routing context; stripping them can downgrade direct job links into company listing pages.
- For ZipRecruiter, do not treat `uuid` as a unique job identity when `lk` is missing. `uuid` can represent a company jobs context reused across multiple roles, so identity must include role context (title/company) to prevent cross-role overwrites and wrong "View Job" targets.
- For ZipRecruiter company pages, role-specific `lk` can be emitted only after selecting the specific role row; base `uuid` links often open the default role. Treat missing-`lk` URLs as unresolved, not final.
- Before claiming ZipRecruiter deep-link fixes, verify that automation actually changes the selected role (detail title must match target title). If the detail pane is stuck on one title, all derived URLs are suspect.
- When a source-native search returns zero for the configured hard filters, reproduce the query through the source's visible search UI before calling the mapping a workaround candidate. If the UI generates the same URL and zero-result state, treat zero as valid native search behavior unless the product requirement explicitly prefers recall over strict source-native filtering.

## Sources Table "Imported" Metric Definition

- The Sources dashboard's per-source `Imported` total must be the count of distinct persisted jobs that survived the hard filter (`listImportedJobCountsBySourceId`, deduped by `normalized_hash`), not a SUM of `source_run_deltas.imported_kept_count` across every historical run (`listSourceRunTotals`). The latter re-counts the same recurring job every time a source is re-captured (every 12h by default), so a source with a handful of real active jobs can show a lifetime "Imported" total in the thousands while the latest-run delta stays in the tens. This is the same anti-pattern flagged earlier in this file ("imported must come from aggregated persisted rows, not raw `source_run_deltas`") — it had regressed back into `aggregateSourceRunTotals`/`server.js`.
- `listSourceRunTotals` (raw per-run SUM) is still legitimate for cumulative *funnel diagnostics* (how much raw volume a source has processed over time) — just don't surface it as if it means "distinct jobs imported." Keep the two concepts in separate fields if both are ever needed on screen.
- The `Imported` metric intentionally ignores downstream `applications.status` (applied/viewed/skipped/rejected) — those are queue-management states, not import/dedupe states. Don't gate the base "was this job imported" count on workflow status; that's a separate concern (see `countActiveJobsByIds` for the workflow-gated variant used elsewhere).

## Env-Override Config vs Test Isolation
